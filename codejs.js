...
// function to get public ip address (as a fallback)
function getPublicIp() {
  fetch("https://geolocation-db.com/json/", {
    method: "GET",
    headers: {},
...
function fetchWeatherDataByCoords(lat, lng) {
  fetch(
    `https://api.opencagedata.com/geocode/v1/json?q=${lat}+${lng}&key=f2bcfb32d1f441e786bcae761a2f81cd`
  )
    .then((response) => response.json())
    .then((data) => {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        getPublicIp(); // Fallback to IP-based location
      }
    );
  } else {
...
document.getElementById("openMapBtn").addEventListener("click", openModal);
document.querySelector(".close").addEventListener("click", closeModal);

// Call getCurrentLocation when the page loads
window.onload = getCurrentLocation;

// Cities array (you can keep your existing cities array here)
const cities = [
  {
    country: "PK",
    name: "Abbottabad",
